...
2. You will connect deb-router-3 to your existing network, per the network diagram above.
...
3. FRR/OSPF will be installed and configured correctly on deb-router-3 to allow full interconnectivity between your new client VM, deb-router-3, all the other VMs on your network, and the Internet.
4. All of the necessary DNS configurations will be added to deb-router-1 to allow for the following mappings:
  - 192.168.175.11 > deb-router-1.ops345.org
  - 192.168.175.12 > deb-router-3.ops345.org
  - 192.168.200.11 > deb-router-3.cnet3.ops345.org
  - 192.168.200.12 > *yourchosendistro*.cnet3.ops345.org
    - Replace "yourchosendistro" with the name of the distro you chose to use.
5. Change deb-router-3 and your new client VM's configurations so that they both looks to deb-router-1 for all DNS queries
6. Provide a one page write up on your chosen distro explaining what it is, who created it, why they created it, and what it is best used for.
...
show ip route ospf
```
- The DNS configration for deb-router-3 and your new client VM (showing that they are looking to deb-router-1 for all DNS queries)
...
- Everything you need to complete this assignment was done in the labs. You just have to apply those concepts to two new VMs. No external information is needed. **DO NOT USE AI FOR THIS ASSIGNMENT**. AI may not necissarily know or understand the context in which these tasks are being completed and may give you configurations that will interfere with your end goals.
...
| Correct FRR/OPSF configuration                | 4      |
| Correct DNS configuration                     | 4      |
| Successful Pings                              | 4      |
| Distro Write Up                               | 4      |
| **Total**                                     | **20** |
